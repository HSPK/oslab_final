...
PUBLIC void task_queue_init(task_queue *t) {
	t->front = 0;
	t->len = 0;
}

PUBLIC int task_top(task_queue *t) {
	return t->task_id[t->front];
}

PUBLIC void task_push(task_queue *t, int tid) {
	t->task_id[(t->front + t->len) % MAX_TASKS] = tid;
	t->len++;
}

PUBLIC int task_get(task_queue *t, int pos) {
	if (pos < 0 || pos >= t->len) return -1;
	return t->task_id[(t->front + pos) % MAX_TASKS];
}

PUBLIC void task_del(task_queue *t, int pos) {
	if (pos < 0 || pos >= t->len) return;
	for (int i = pos; i > 0; --i) {
		t->task_id[(t->front + i) % MAX_TASKS] = 
			t->task_id[(t->front + i  + MAX_TASKS - 1) % MAX_TASKS];
	}
	t->front = (t->front + 1) % MAX_TASKS;
	t->len--;
}

PUBLIC int task_popf(task_queue *t) {
	int ret = t->task_id[t->front];
	t->front = (t->front + 1) % MAX_TASKS;
	t->len--;
	return ret;
}

PUBLIC struct proc* find_next_rr(task_queue *t, task_queue *tn, int ticks_new) {
	while (t->len) {
		int tid = task_top(t);
		if (proc_table[tid].p_flags) {
			task_popf(t);
			task_push(&task_ready_queue, tid);
			continue;
		}
		if (proc_table[tid].ticks > 0) {
			return &proc_table[tid];
		} else {
			int tid = task_popf(t);
			proc_table[tid].ticks = ticks_new;
			task_push(tn, tid);
		}
	}
	return 0;
...
void print_proc_table() {
	for (int i = 0; i < 1; i++) {
		if (proc_table[i].name[0] == 0) break;
		printl("proc : %d %d %s\n", i, proc_table[i].p_flags, proc_table[i].name);
	}
}

// void schedule() {
// 	const int ticks_q1 = 3;
// 	const int ticks_q2 = 5;
// 	const int ticks_q3 = 10;
// 	/* 将就绪队列中所有进程移入队列1 */
// 	print_proc_table();
// 	print_proc_table();
// 	panic("will never run here\n");
// 	for (int i = 0; i < task_ready_queue.len; i++) {
// 		int tid = task_get(&task_ready_queue, i);
// 		assert(tid != -1);
// 		// printl("[schedule] : pid %d scheduled %d flag : %d\n", tid, i, proc_table[tid].p_flags);
// 		if (!proc_table[tid].p_flags) {
// 			task_del(&task_ready_queue, i--);
// 			proc_table[tid].ticks = ticks_q1;
// 			task_push(&task_q1, tid);
// 		}
// 	}
// 	// if (proc_table[0].p_flags == 0)
// 	// 	panic("will never run here\n");
// 	struct proc *p;
// 	if ((p = find_next_rr(&task_q1, &task_q2, ticks_q2)) != 0) {
// 		p_proc_ready = p;
// 		return;
// 	}	
	
// 	// __asm__("xchg %%bx,%%bx"::);
// 	if ((p = find_next_rr(&task_q2, &task_q3, ticks_q3)) != 0) {
// 		p_proc_ready = p;
// 		return;
// 	}	

// 	if ((p = find_next_rr(&task_q3, &task_q3, ticks_q3)) != 0) {
// 		p_proc_ready = p;
// 		return;
// 	}	
